...
def cpp_code():
    ...
    response_dict = code_editor(your_code_string,
                                ...
                                key="my_editor",
                                allow_reset=True,
                                lang='cpp')  # Change lang to 'cpp'
    print(response_dict)
    if response_dict['text'] != your_code_string:
        with st.spinner("Compiling your code..."):
            with open("tempCode/temp.cpp", "w") as f:
                f.write(response_dict['text'])
            os.system("g++ tempCode/temp.cpp -o tempCode/output")
            os.system("tempCode/output > tempCode/output.txt")
            with open("tempCode/output.txt", "r") as f:
                output = f.read()
            with st.expander("Output"):
