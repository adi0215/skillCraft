...
def record_audio(filename, stop_event, audio):
...
def mainGPT(assistant_id, thread_id, client, model, audio):
    ...
    with stylable_container(
        ...
        if 'stop_event' not in st.session_state or 'record_thread' not in st.session_state:
            ...
            st.session_state['record_thread'] = threading.Thread(target=record_audio, args=(WAVE_OUTPUT_FILENAME, st.session_state['stop_event'], audio))
